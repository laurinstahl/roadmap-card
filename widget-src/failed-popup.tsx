/* 
  ...
  - check this widget to see how a toast works with an UI: https://github.com/figma/widget-samples/blob/main/WidgetToast/widget-src/code.tsx
  - how to handle user events: https://www.figma.com/widget-docs/handling-user-events/
  - property menu: https://www.figma.com/widget-docs/api/type-PropertyMenu
  - property menu 2: https://www.figma.com/widget-docs/handling-user-events/#property-menus
  - chat gpt conversation called "User Request Summarized."
...
const { widget } = figma
const { useSyncedState, useEffect, usePropertyMenu, AutoLayout, Input, Frame, Text, SVG } = widget

function Widget() {
  const [statusOption, setStatus] = useSyncedState<string>('status', '#e06666');
  const [priority, setPriority] = useSyncedState<string>('priority', 'p1');
  const [theme, setTheme] = useSyncedState<string>('theme', 'marketplace');
  ...
  const [showSettings, setShowSettings] = useSyncedState<boolean>("showSettings", false);
  ...
  const themeOptions = [
    { option: 'creator', label: '⚙️ Creator Tools' },
    { option: 'incentive', label: '👷 Creator Incentive' },
    { option: 'marketplace', label: '🔄 Marketplace' },
  ];
  const statusOptions = [
  ...
  const themeColors = [
    { option: 'creator', label: '⚙️ Creator Tooling', color: "#CCF1FACC", textColor: "#00B8E6" },
    { option: 'incentive', label: '👷 Creator Incentive', color: "#F6E8FF", textColor: "#7B61FF" },
    { option: 'marketplace', label: '🔄 Marketplace', color: "#D4EDDDCC", textColor: "#2AA653" },
  ];
  const statusColors = [
  ...
  const statusSvg = `
    ...
    <path
      ...
      fill="${statusColors.find((f) => f.option === statusOption)?.textColor}"></path>
    </svg>
  `;
