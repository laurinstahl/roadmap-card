...
function Widget() {
  ...
  return (
    ...
    > 
      ...
      <AutoLayout fill={statusColors.find((f) => f.option === statusOption)?.color} cornerRadius={4} padding={4} spacing={4} width={300} horizontalAlignItems={"center"} verticalAlignItems={"center"}>
          <Text fontSize={8} fontWeight={400} fill={statusColors.find((f) => f.option === statusOption)?.textColor}>{statusColors.find((f) => f.option === statusOption)?.tooltip}</Text>
          <SVG
            src={statusSvg}
          />
      </AutoLayout>
